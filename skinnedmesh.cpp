...
SkinnedMesh::SkinnedMesh(TextureLoader& textLoader): currentAnimation(0), loader(textLoader)
{
	m_VAO = 0;
	...
	for (unsigned int i = 0; i < pScene->mNumMaterials; ++i)
	{
		...
		if (pMaterial->GetTextureCount(aiTextureType_DIFFUSE) > 0)
		{
			...
			if (pMaterial->GetTexture(aiTextureType_DIFFUSE, 0, &Path, NULL, NULL, NULL, NULL, NULL) == AI_SUCCESS)
			{
				...
				if (!loader.loadedTextures)
				{
					loader.TextureFromFile(p.c_str(), Dir);
				}

				m_Textures[i] = loader.textures[i];
			}
		}
	}
	loader.loadedTextures = true;
	return ret;
}
...
void SkinnedMesh::Draw(Shader& shader)
{
	...
	for (unsigned int i = 0; i < m_Entries.size(); i++) {
		...
		if (m_Textures[MaterialIndex]) {
			...
			GLint textureLocation = glGetUniformLocation(shader.GetProgram(), "texture1");
...
void SkinnedMesh::setBoneTransformations(GLfloat currentTime, Shader& shader)
{
	std::vector<glm::mat4> Transforms;
	boneTransform((float)currentTime, Transforms);
	glUniformMatrix4fv(glGetUniformLocation(shader.GetProgram(), "gBones"), (GLsizei)Transforms.size(), GL_FALSE, glm::value_ptr(Transforms[0]));
}
